...
def policy_evaluation(mdp: TabularMDP, policy: List[Action], theta: float = 1e-8, max_iters: int = 100_000) -> List[float]:
...
def policy_improvement_from_v(mdp: TabularMDP, V: List[float]) -> List[Action]:
    # Greedy improvement using V (one-step lookahead)
    new_policy = []
    for s in range(mdp.nS):
        best_a = 0
        best_val = -math.inf
...
def policy_improvement_from_q(mdp: TabularMDP, Q: List[List[float]]) -> List[Action]:
    # Greedy improvement directly using Q-values
    return greedy_policy_from_q(Q)
...
def policy_iteration(
    ...
    # Start with arbitrary policy (all zeros)
    ...
    for _ in range(max_policy_iters):
        ...
        # Improvement using V and Q (both required by V1)
        improved_v = policy_improvement_from_v(mdp, V)
        improved_q = policy_improvement_from_q(mdp, Q)

        # Choose one (Q-greedy is explicit)
        new_policy = improved_q

        if new_policy == policy and improved_v == policy:
            return policy, V, Q
